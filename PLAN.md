...
| 6 | Harness Ledger MCP over the same paths as the UI (adapter/improvements/beats), remove raw-store and dead checkpoint tools, remove `harness/src/web/server.ts`, MCP protocol smoke test, permission parity tests | none | — | 1b, 2, 4, 5, 7 | subagent — files: `harness/src/mcp-server.ts`, `harness/test/mcp-server.test.ts` (new), `harness/test/verification.test.ts` (mcp structural test), `.mcp.json`, delete `harness/src/web/` |
