...
process.env.HARNESS_DB_PATH = join(mkdtempSync(join(tmpdir(), "harness-knowledge-test-")), "harness.db");
...
const { composeManagedKnowledge, HARNESS_START, HARNESS_END, MalformedMarkersError, sha256, KNOWLEDGE_CAP } = knowledge;
...
test("composer preserves text outside the markers byte-for-byte (before, after, unicode, trailing newlines)", () => {
  ...
  const out = composeManagedKnowledge(current, [{ id: 2, instruction: "B" }, { id: 1, instruction: "A" }]);
  ...
  assert.equal(out.managed_block, `${HARNESS_START}\n${knowledge.MANAGED_HEADING}\n- A\n- B\n${HARNESS_END}`);
...
test("composer throws on malformed markers instead of overwriting", () => {
  ...
  assert.throws(() => composeManagedKnowledge(`${HARNESS_END} x ${HARNESS_START}`, []), MalformedMarkersError);
  assert.throws(() => composeManagedKnowledge(`${HARNESS_START} ${HARNESS_END} ${HARNESS_START} ${HARNESS_END}`, []), MalformedMarkersError);
});

test("composer flags the 9,000-character cap", () => {
  const out = composeManagedKnowledge("x".repeat(KNOWLEDGE_CAP - 10), [{ id: 1, instruction: "y".repeat(50) }]);
...
function makeImprovement(instruction: string) {
  ...
  const ep = store.createTaskEpisode({ project_id: PROJECT, title: "ep", provenance: "manual", evidence_history_item_ids: [item.id] }) as { id: number };
...
test("accept without a snapshot: choice recorded, write_status stays none, previews null", () => {
  const out = improvements.improvementAction({ action: "accept", id: A.correctionId, destination: "project" });
  ...
  const rule = db.prepare(`SELECT evidence_level FROM rules WHERE id = ?`).get(A.ruleId) as { evidence_level: string };
  assert.equal(rule.evidence_level, "human_grounded");
});

test("skill destination is rejected with a clear error", () => {
  assert.throws(() => improvements.improvementAction({ action: "accept", id: A.correctionId, destination: "skill" }), /Skill isn't available yet/);
});
...
test("accept with a snapshot creates exactly one pending version whose content equals the preview", () => {
  store.recordKnowledgeSnapshot({ target: "project", project_id: PROJECT, content: USER_KNOWLEDGE, fetched_by: "test" });
  const out = improvements.improvementAction({ action: "accept", id: A.correctionId, destination: "project" });
  const preview = out.lovable.previews.project!;
  assert.equal(preview.current_user_text, USER_KNOWLEDGE);
  assert.ok(preview.managed_block.includes("- Do not enable recurring background work by default."));
  assert.equal(preview.final_content, `${USER_KNOWLEDGE}\n\n${preview.managed_block}`);
  assert.equal(out.lovable.write_status, "pending");
  assert.equal(out.stages.find((s) => s.key === "in_lovable")!.note, "Waiting for Harness to add it");
  assert.equal(out.stages.find((s) => s.key === "proof")!.note, "Not proven yet");
  const pending = store.listPendingKnowledgeWrites() as { id: number; new_content: string; previous_sha256: string }[];
...
test("stale path: executor reports live content changed; nothing is written", () => {
  ...
  assert.equal((db.prepare(`SELECT state FROM rules WHERE id = ?`).get(A.ruleId) as { state: string }).state, "approved");
});
...
test("read-back hash match marks written, activates the rule, and completes In Lovable", () => {
  ...
  assert.equal((db.prepare(`SELECT state FROM rules WHERE id = ?`).get(A.ruleId) as { state: string }).state, "active");
  ...
  const events = (db.prepare(`SELECT kind FROM events WHERE kind = 'rule.applied'`).all() as { kind: string }[]).length;
  assert.ok(events >= 1);
  void out;
});
...
test("restore creates a NEW pending version pointing at the restored one, with the earlier content; history untouched", () => {
  ...
  const out = improvements.improvementAction({ action: "restore", id: A.correctionId, version_id: writtenVersionId });
  ...
  assert.equal((db.prepare(`SELECT state FROM rules WHERE id = ?`).get(A.ruleId) as { state: string }).state, "rolled_back");
  assert.throws(() => improvements.improvementAction({ action: "restore", id: A.correctionId, version_id: 999999 }), /does not belong/);
});
...
test("reopen / skip cancel any pending write so the executor never applies a stale decision", () => {
  ...
  assert.equal((store.listPendingKnowledgeWrites() as { rule_id: number }[]).filter((p) => p.rule_id === B.ruleId).length, 1);
  improvements.improvementAction({ action: "reopen", id: B.correctionId });
  assert.equal((store.listPendingKnowledgeWrites() as { rule_id: number }[]).filter((p) => p.rule_id === B.ruleId).length, 0);
  improvements.improvementAction({ action: "accept", id: B.correctionId, destination: "project" });
...
test("workspace preview needs the project's workspace id; composed from the workspace snapshot", () => {
  ...
  store.recordKnowledgeSnapshot({ target: "workspace", workspace_id: WORKSPACE, content: "", fetched_by: "test" });
...
test("no Lovable import and no network call in the knowledge path", () => {
  for (const file of ["../src/knowledge.ts", "../src/improvements.ts", "../src/store.ts", "../src/mcp-server.ts"]) {
    ...
    assert.ok(!/set_project_knowledge|set_workspace_knowledge|setProjectKnowledge|setWorkspaceKnowledge/.test(code), `${file} must not write Knowledge itself`);
  }
});
