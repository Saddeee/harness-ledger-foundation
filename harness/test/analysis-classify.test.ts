...
test("humanVisibleText: Lovable's current replies put the visible text after the tool-use blocks -- return that, never the markup (both copies agree)", async () => {
  // Verbatim shape of a 2026-09-13 reply: no message_user blocks at all.
  const raw =
  ...
  // Markup only: nothing a person saw.
  const onlyTools = '<lov-tool-use id="a" name="code--exec" data="{}">\n</lov-tool-use>';
  assert.equal(replyText.humanVisibleText(onlyTools), "");
});

test("classifier: a message asking for the opposite of a live rule opens a Retire/Keep proposal ('you asked for the opposite'), whatever it is classified as", async () => {
  // Live: "switch to euros" and "make everything lowercase" were new_task
  // messages against the live kronor and sentence-case rules; nothing noticed.
