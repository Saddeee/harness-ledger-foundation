...
Run it: Node 22 (`nvm use`), `npm i`, `npm run harness:install && npm run harness:build`, then `HARNESS_RUNTIME=local HARNESS_DB_PATH=harness/data/harness.db npm run dev` and, in another terminal, `npm run harness:executor` (the loop). Tests: `cd harness && npm test` (339 pass at head), `npm run typecheck` in both packages, `npm run build`. Repo-wide `npm run lint` has ~1,400 pre-existing Prettier errors in files this work never touched; the rule was "every touched file is eslint-clean", and it is.

## 3. What was built, round by round (all on this branch)
...
## 4. Current state of the owner's data (read-only facts at handoff)
...
## 5. What is NOT built (and the owner knows)
...
## 6. How the owner wants work done (binding)
...
## 7. Lessons from this session (so you don't repeat them)
