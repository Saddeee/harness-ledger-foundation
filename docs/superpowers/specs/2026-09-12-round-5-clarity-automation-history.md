...
Date: 2026-09-12. Status: DRAFT for the owner's review. Follows the owner's feedback of 2026-09-12 on the round-4 state (see `docs/HANDOFF.md`). Everything here is local-runtime only, like rounds 2–4.
...
### 3b. History timeline (per target)

The owner's "git graph". The history is linear per target, so it is a vertical timeline, newest first, not a branch graph. One timeline per project and one for the workspace. Node kinds:
...
API: the knowledge route's `KnowledgeTargetView` gains `timeline: TimelineNode[]` with `{ id, kind, at, label, actor, summary, content: string | null, diff: KnowledgeChanges | null, rule_ids: number[], restored_from: number | null, improvement_id: number | null }`. Contents are sent in full (≤ 10 k chars each; a project with 40 versions is ~400 kB, acceptable for a local app; the route caps the timeline at the newest 200 nodes). The Skills page keeps its per-skill sections but its "History" list now links to the workspace timeline node.
...
3. **Adherence check (tokens, opt-in, next round).** For each applicable episode after the write, a judge model reads Lovable's human-visible reply and the rule and answers `followed / broke / not applicable` with a quoted line. Gives "Followed in 5 of 6 builds" and, crucially, distinguishes "rule not exercised" from "rule followed". This is the missing middle between observation and a paired test. Designed here, built after Phase B is decided; it reuses the classifier's context builder and the token budget.
4. **Paired test (credits, per item, Phase B, unchanged).** The only way to show a counterfactual: the same request with and without the rule in a temporary copy. Remains the owner's decision per item.
...
- New paragraph under the intro: **"Credits left this month? Spend a few on making Lovable better at your project. Syncing chats and writing Knowledge costs nothing; testing one rule in a temporary copy costs about 1.5 credits."** No live balance (402 on Pro; see §0). If the credits endpoint ever answers 200 for the executor's grant, the in-app Inbox notice can show "You have N credits left this month" — designed, not built; the executor gains a `getCredits()` that returns `null` on 402 so this is one UI line later.
...
- Suggestions (9): 3 pending (one flagged "Harness wasn't sure"), 1 retirement proposal, 2 in Lovable with `rule_health` rows and one verdict each, 1 accepted automatically, 1 reverted, 1 skipped, 1 needing attention (stale write).
...
- Tests: `demo.test.ts` asserts add → remove returns every affected table to its prior row count, and that add is idempotent. Removal is by the gathered id sets, extended for `rule_verdicts`, `rule_health`, `retire_proposals`, `message_classifications`, `analysis_*` rows it created.
- No second fake project: the executor would try to sync it from Lovable every hour and fail.

## 8. Out of scope this round

Phase B (paired test), the adherence judge (§5.3), a hosted runtime, a Scoreboard page, e-mail notifications, a no-model correction detector (the owner did not ask for it after §1 clarified that analysis should use a model).
...
1. **Vocabulary** (Haiku): nav/copy "Improvements → Suggestions", "Miner → Rule writer", landing copy (§6), test pins, HANDOFF glossary. Migration v11 for the `llm_models` key rename and the file rename (Sonnet).
2. **Knowledge API: rules table fields + timeline** (Sonnet; owns migration v12 `rule_verdicts`): `KnowledgeActiveRule.status/since`, `timeline` builder in `improvements.ts` (delimited block), verdict store functions and the `verdict` action.
3. **Instructions page UI** (Sonnet): rules table, collapsed full text, timeline component with select-to-show-full-text and diff toggle; Skills page history link.
4. **Inbox compact card + navigation** (Sonnet): §2.
5. **Automatic mode** (Sonnet): setting, Settings section, `runAnalysis` hook, Inbox "wasn't sure" line, marker in Suggestions, timeline `decision` nodes.
6. **Proof wording + verdict UI** (Sonnet): §5.1 copy, §5.2 buttons on Instructions and Suggestions detail, retirement signal coupling, Details paragraph.
7. **Demo data** (Sonnet): §7 (after 2, 5 so it can seed verdicts and automatic decisions).
...
## 10. Owner decisions to confirm

1. "Suggestions" as the page name (alternatives considered: "Proposals", "Ledger"; "Rules" would collide with the Instructions page).
2. Automatic mode threshold 0.8 default, global setting, still gated by per-project auto-write.
3. "helped" leaves the UI in favour of "builds without a repeat correction"; the one-click verdict is added.
4. Landing page credits line without a live balance.
