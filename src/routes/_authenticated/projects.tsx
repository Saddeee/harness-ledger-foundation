...
export const Route = createFileRoute("/_authenticated/projects")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Connect Lovable and choose which projects to work on." },
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant={conn ? "outline" : "default"} onClick={connect} disabled={busy !== null}>
        ...
        <div className="text-sm text-muted-foreground">
          {connection.isLoading ? (
            ...
            <span>
              Connected as <span className="text-foreground">{conn.email ?? conn.lovable_user_id}</span>{" "}
              ...
              <div className="flex flex-wrap items-center gap-6">
                ...
                <label className="flex items-center gap-2 text-sm">
                  <Switch
                    ...
                    onCheckedChange={(v) =>
                      updateWorkspace.mutate({ id: ws.id, patch: { can_write_workspace_knowledge: v } })
                    }
                  />
                ...
                <label className="flex items-center gap-2 text-sm">
                  <Switch
                    ...
                    onCheckedChange={(v) => updateWorkspace.mutate({ id: ws.id, patch: { can_write_skills: v } })}
                  />
                  Write Skills
                </label>
              </div>
                    ...
                    wsProjects.map((p) => (
                      <TableRow key={p.id}>
                        <TableCell>
                          ...
                          <div className="text-xs text-muted-foreground">{p.lovable_project_id}</div>
                        </TableCell>
                        <TableCell>
                          <Switch
                            checked={!!p.can_read}
                            onCheckedChange={(v) =>
                              updateProject.mutate({
                                ...
                                patch: v ? { can_read: true } : { can_read: false, can_write_knowledge: false, can_replay: false },
                              })
                            }
                          />
                        </TableCell>
                        <TableCell>
                          <Switch
                            ...
                            disabled={!p.can_read}
                            onCheckedChange={(v) => updateProject.mutate({ id: p.id, patch: { can_write_knowledge: v } })}
                          />
                        </TableCell>
                        <TableCell>
                          <Switch
                            ...
                            disabled={!p.can_read}
                            onCheckedChange={(v) => updateProject.mutate({ id: p.id, patch: { can_replay: v } })}
                          />
                        </TableCell>
                      </TableRow>
                    ))
